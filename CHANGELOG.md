...
### Changed
- GitHub is now **off by default** (`github.enabled: false` in both the runtime defaults and the first-run config template), matching PagerDuty. Set `"enabled": true` under `github` to turn it on; existing config files are not affected
...
- docs: step-by-step guide to creating the PagerDuty **User API Token** (where the page is, the direct URL pattern, why not to use an admin General Access key, how to verify and revoke) in the TUTORIAL and README, plus a troubleshooting entry for `PagerDuty: ⚠ HTTP 401`
