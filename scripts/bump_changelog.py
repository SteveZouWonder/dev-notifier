...
def build_release_notes(
    ...
    notes = (
        f"Menu-bar / tray notifier for Jira, GitHub & PagerDuty (macOS + Windows).\n"
        f"\n"
        f"## Download & Install\n"
        f"\n"
        f"| Platform | File | How to install |\n"
        f"|----------|------|----------------|\n"
        f"| macOS (Apple Silicon) | `DevNotifier-{version}.dmg` | Open, drag into Applications, then see the note below |\n"
        f"| Windows | `DevNotifier-{version}-setup.exe` | Run the installer (per-user; SmartScreen: More info -> Run anyway) |\n"
        f"| Windows (portable) | `DevNotifier-{version}-portable.exe` | Just run it |\n"
        f"\n"
        f"> Verify file integrity via the attached `SHA256SUMS.txt` (the in-app "
        f"updater does this automatically).\n"
        f"\n"
        f"## macOS note (first launch and after every update)\n"
        f"This is a free, open-source app **without an Apple Developer certificate**,\n"
        f"so it is not notarized and macOS blocks it with \"is damaged and can't be\n"
        f"opened\". Clear the quarantine flag once in Terminal:\n"
        f"```\n"
        f"xattr -dr com.apple.quarantine /Applications/DevNotifier.app\n"
        f"```\n"
        f"or use **System Settings -> Privacy & Security -> Open Anyway** after the\n"
        f"first attempt. (On macOS 14 and earlier, right-click -> Open also works.)\n"
        f"\n"
        f"---\n"
        f"## What's changed\n"
        f"\n"
